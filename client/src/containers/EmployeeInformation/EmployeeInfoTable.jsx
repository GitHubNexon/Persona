...
import EmployeeFormModal from "../../Modal/EmployeeFormModal";
import EmployeeExpandedInfo from "./EmployeeExpandedInfo";
...
const EmployeeInfoTable = () => {
  ...
  const columns = [
    ...
    {
      name: "Gender",
      cell: (row) => [row.basicInfo?.gender].filter(Boolean).join(" ") || "-",
      width: "200px",
      sortable: false,
      align: "left",
    },
    {
      ...
      cell: (row) =>
        [
          row.contactInfo?.address?.zip,
          row.contactInfo?.address?.street,
          row.contactInfo?.address?.city,
          row.contactInfo?.address?.barangay,
          row.contactInfo?.address?.region,
          row.contactInfo?.contactNumber,
        ]
          .filter(Boolean)
          .join(" ") || "-",
      width: "200px",
    ...
    },
    {
      name: "Employee Info",
      cell: (row) =>
        [row.employeeInfo?.employeeId, row.employeeInfo?.position]
          .filter(Boolean)
          .join(" - ") || "-",
      width: "200px",
      sortable: false,
  ...
  const renderExpandedContent = (rowData) => (
    <>
      <EmployeeExpandedInfo data={rowData} />
    </>
  );
